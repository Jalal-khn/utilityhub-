...
export default function PdfToJpg() {
  ...
  const convertPdfToJpg = async () => {
    ...
    try {
      const rendered = await pdfToBlobs(file, "image/jpeg", 1.5);
      setBlobs(rendered);
      setPageCount(rendered.length);
      setPreviewUrl(URL.createObjectURL(rendered[0]));
    } catch (error) {
      alert("Error converting PDF to JPG: " + (error as Error).message);
    } finally {
  ...
  const downloadAll = async () => {
    ...
    blobs.forEach((blob, index) => {
      zip.file(`${base}-page-${index + 1}.jpg`, blob);
    });
    ...
    <div className="space-y-6">
      ...
      {file && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <Button onClick={convertPdfToJpg} disabled={isProcessing} className="w-full">
              {isProcessing ? (
                <>
